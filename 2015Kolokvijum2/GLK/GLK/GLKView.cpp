...
{
	if (nChar == VK_RIGHT) m_glRenderer.eyeAlfa -= 0.1;
	if (nChar == VK_LEFT) m_glRenderer.eyeAlfa += 0.1;
	if (nChar == VK_UP) m_glRenderer.eyeBeta += 0.1;
	if (nChar == VK_DOWN) m_glRenderer.eyeBeta -= 0.1;
	if (nChar == VK_ADD) m_glRenderer.eyeR += 2;
	if (nChar == VK_SUBTRACT) m_glRenderer.eyeR -= 2;
	if (nChar == 'Q') m_glRenderer.moonr -= 5.0;
	if (nChar == 'W') m_glRenderer.moonr += 5.0;
	if (nChar == 'S') m_glRenderer.ukljuciS = !m_glRenderer.ukljuciS;
	Invalidate();
