...
void CGLKView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	switch (nChar) {
	case '1':
		m_glRenderer.angleArm1 += 1;
		break;
	case '2':
		m_glRenderer.angleArm1 -= 1;
		break;
	case '3':
		m_glRenderer.angleArm2 += 1;
		break;
	case '4':
		m_glRenderer.angleArm2 -= 1;
		break;
	case '5':
		m_glRenderer.angleHead += 1;
		break;
	case '6':
		m_glRenderer.angleHead -= 1;
		break;
	...
	case VK_LEFT:
		m_glRenderer.alfaCam += 0.1;
		break;
	case VK_RIGHT:
		m_glRenderer.alfaCam -= 0.1;
		break;
	case VK_UP:
		if(m_glRenderer.betaCam < 1.5)
			m_glRenderer.betaCam += 0.1;
		break;
	case VK_DOWN:
		if (m_glRenderer.betaCam > 0.1)
			m_glRenderer.betaCam -= 0.1;
		break;
	case VK_SUBTRACT:
		m_glRenderer.rCam -= 1;
