...
void CGLKView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	switch (nChar) {
	case 'A':
		if (!m_glRenderer.animating) {
			m_glRenderer.animating = true;
			SetTimer(1, 30, NULL);
		}
		break;
	case 'S':
		if (m_glRenderer.animating){
			m_glRenderer.animating = false;
			SetTimer(2, 30, NULL);
		}
		break;
	case VK_RIGHT:
		m_glRenderer.cam_alpha += 5.0;
		break;
	case VK_LEFT:
		m_glRenderer.cam_alpha -= 5.0;
		break;
	case VK_UP:
		if (m_glRenderer.cam_beta < 90) 
			m_glRenderer.cam_beta += 5.0;
		break;
	case VK_DOWN:
		if (m_glRenderer.cam_beta > -90) 
			m_glRenderer.cam_beta -= 5.0;
		break;
	case VK_ADD:
		if (m_glRenderer.cam_dist > 2.0) 
			m_glRenderer.cam_dist -= 1.0;
		break;
...
void CGLKView::OnTimer(UINT_PTR nIDEvent)
{
	// TODO: Add your message handler code here and/or call default
	if (nIDEvent == 1 && m_glRenderer.animating) {
		m_glRenderer.angleRF += 1;
		if (m_glRenderer.angleRF >= 7) {
			KillTimer(1);
			//m_glRenderer.animating = false;
		}
		Invalidate(FALSE); // ponovo iscrtaj
	}
	if (nIDEvent == 2 && !m_glRenderer.animating) {
		m_glRenderer.angleRF -= 1;
		if (m_glRenderer.angleRF <= 0) {
			KillTimer(2);
			//m_glRenderer.animating = false;
		}
		Invalidate(FALSE); // ponovo iscrtaj
